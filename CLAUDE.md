...
- **Multi-tenant demo data**: `forno-api`'s seed script (`prisma/seed.ts` in that repo) creates an `Enterprise` stamped with the same `DEMO_ENTERPRISE_ID` the old in-memory seed used, plus `admin.demo`/`tecnico.demo` test users — kept consistent so demo data lines up across both repos. Logging in with a different real company shows empty lists until that company has its own data in `forno-api`.
