  ...
  return (
    <Screen>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => setStoreId(item.id)} style={[styles.chip, storeId === item.id && styles.chipSelecionado]}>
            <Text style={[styles.chipTexto, storeId === item.id && styles.chipTextoSelecionado]}>{item.description}</Text>
          </Pressable>
        )}
      />

      <FilterInput valor={filtro} aoMudar={(t) => { setFiltro(t); if (storeId) carregar(storeId, t); }} placeholder="Filtrar por descrição ou patrimônio..." />
      ...
      <PrimaryButton titulo="+ Novo forno" onPress={() => router.push("/fornos/novo-forno")} style={{ marginTop: spacing.sm }} />
    </Screen>
  );
}
