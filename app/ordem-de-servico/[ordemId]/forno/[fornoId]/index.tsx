      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <>
            ...
            {jaRegistradas.map((m) => (
              <View key={m.id} style={styles.itemRegistrado}>
                <Text style={styles.itemRegistradoTexto}>{nomePeca(m.partId)} · {m.serviceType}</Text>
                {m.observation ? <Text style={styles.itemRegistradoObs}>{m.observation}</Text> : null}
              </View>
            ))}
          </>
        ) : null}
        ...
        {pecasDoForno.length === 0 ? (
          ...
          <View style={styles.chips}>
            {pecasDoForno.map((p) => (
              <Pressable key={p.id} onPress={() => setPartId(p.id)} style={[styles.chip, partId === p.id && styles.chipSelecionado]}>
                <Text style={[styles.chipTexto, partId === p.id && styles.chipTextoSelecionado]}>{p.reference} · {p.description}</Text>
              </Pressable>
            ))}
          </View>
        )}
        ...
        <View style={styles.chips}>
          {SERVICE_TYPES.map((s) => (
            <Pressable key={s} onPress={() => setServico(s)} style={[styles.chip, servico === s && styles.chipSelecionado]}>
              <Text style={[styles.chipTexto, servico === s && styles.chipTextoSelecionado]}>{s}</Text>
            </Pressable>
          ))}
        </View>

        <TextField rotulo="Observação" value={observacao} onChangeText={setObservacao} placeholder="O que foi feito nesta peça" />

        {erro ? <Text style={styles.erro}>{erro}</Text> : null}
        <PrimaryButton titulo="+ Adicionar peça à lista" variante="secundaria" onPress={adicionarItem} />
          ...
          <>
            ...
            {pendentes.map((item, index) => (
              <View key={index} style={styles.itemPendente}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.itemPendenteTexto}>{nomePeca(item.partId)} · {item.serviceType}</Text>
                  {item.observation ? <Text style={styles.itemRegistradoObs}>{item.observation}</Text> : null}
                </View>
...
const styles = StyleSheet.create({
  ...
  itemRegistrado: { backgroundColor: colors.highlight, borderRadius: radius.sm, padding: spacing.sm, marginBottom: spacing.xs },
  ...
  itemPendente: { flexDirection: "row", alignItems: "center", backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: radius.sm, padding: spacing.sm, marginBottom: spacing.xs },
