...
import { workOrderUseCase, ovenUseCase, storeUseCase, pdfGenerator } from "../../../infra/ioc/container";
...
export default function DetalheOrdem() {
  ...
  const carregar = useCallback(async () => {
    ...
    const comFornos = await Promise.all(
      orderOvens.map(async (oo) => ({ orderOven: oo, oven: (await ovenUseCase.findById(oo.ovenId))! }))
    );
  ...
  async function baixarPdf() {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        Alert.alert("Disponível na web", "O download de PDF está disponível na versão web do app por enquanto.");
      }
    } finally {
      setGerandoPdf(false);
    }
  ...
  return (
    <Screen>
      ...
      <PrimaryButton titulo="Baixar PDF da ordem" variante="secundaria" onPress={baixarPdf} carregando={gerandoPdf} style={{ marginTop: spacing.md }} />

      {ordem?.status === "pendente" ? (
        <View style={styles.acoes}>
          <PrimaryButton titulo="Finalizar ordem" onPress={finalizar} carregando={finalizando} style={{ flex: 1 }} />
          <PrimaryButton titulo="Cancelar ordem" variante="perigo" onPress={cancelar} carregando={cancelando} style={{ flex: 1 }} />
        </View>
      ) : null}
    </Screen>
  );
