...
export default function NovaOrdem() {
  ...
  function alternarForno(ovenId: number) {
    setSelecoes((atual) => {
      ...
      return { ...atual, [ovenId]: { selecionado: !anterior?.selecionado, observacao: anterior?.observacao ?? "" } };
    });
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {fornos.length === 0 ? (
          ...
          fornos.map((forno) => {
            ...
            return (
              <View key={forno.id} style={[styles.fornoCard, sel?.selecionado && styles.fornoCardSelecionado]}>
                <Pressable style={styles.fornoCabecalho} onPress={() => alternarForno(forno.id)}>
                  ...
                  <Text style={styles.fornoTitulo}>{forno.assetNumber || "s/ patrimônio"} · {forno.description}</Text>
                </Pressable>
                {sel?.selecionado ? (
                  <TextField rotulo="Observação" value={sel.observacao} onChangeText={(t) => mudarObservacao(forno.id, t)} placeholder="Ex: forno está com problema no aquecimento" />
                ) : null}
              </View>
            );
          })
        ...
        <PrimaryButton titulo="Criar ordem de serviço" onPress={salvar} carregando={salvando} style={{ marginTop: spacing.sm, marginBottom: spacing.xl }} />
      </ScrollView>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  ...
  fornoCard: { backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.sm },
  fornoCardSelecionado: { borderColor: colors.primary },
  fornoCabecalho: { flexDirection: "row", alignItems: "center", gap: spacing.sm, marginBottom: spacing.xs },
