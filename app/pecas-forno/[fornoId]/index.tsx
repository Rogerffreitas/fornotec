...
export default function PecasDoForno() {
  ...
  const carregar = useCallback(async () => {
    const [todas, associacoes] = await Promise.all([partUseCase.findAll(), ovenUseCase.findPartsOfOven(id)]);
    setTodasPecas(todas);
  ...
  function alternarSelecao(partId: number) {
    setSelecionadas((atual) => (atual.includes(partId) ? atual.filter((i) => i !== partId) : [...atual, partId]));
  }
    ...
    <Screen>
      ...
      {pecasJaLigadas.length === 0 ? (
        ...
        pecasJaLigadas.map((p) => (
          <View key={p.id} style={styles.linhaLigada}>
            <Text style={styles.linhaLigadaTexto}>{p.reference} · {p.description}</Text>
          </View>
        ))
      )}
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <Pressable onPress={() => alternarSelecao(item.id)} style={[styles.itemSelecionavel, marcada && styles.itemMarcado]}>
              <View style={[styles.checkbox, marcada && styles.checkboxMarcado]} />
              <View style={{ flex: 1 }}>
                <Text style={styles.itemTitulo}>{item.reference} · {item.description}</Text>
...
const styles = StyleSheet.create({
  ...
  linhaLigada: { backgroundColor: colors.highlight, borderRadius: radius.sm, padding: spacing.sm, marginBottom: spacing.xs },
  linhaLigadaTexto: { fontSize: 13, color: colors.primaryDark },
  itemSelecionavel: { flexDirection: "row", alignItems: "center", gap: spacing.sm, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.sm },
