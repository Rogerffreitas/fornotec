...
export default function Pecas() {
  ...
  return (
    <Screen>
      <FilterInput valor={filtro} aoMudar={handleFiltro} placeholder="Filtrar por descrição ou referência..." />
      <FlatList
        data={pecas}
        ...
        renderItem={({ item }) => (
          <ListRow titulo={`${item.reference} · ${item.description}`} subtitulo={findLocation(item.location).description} />
        )}
      />
      <PrimaryButton titulo="+ Nova peça" onPress={() => router.push("/pecas/nova-peca")} style={{ marginTop: spacing.sm }} />
    </Screen>
  );
}
