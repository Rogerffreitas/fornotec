...
export default function NovaPeca() {
  ...
  return (
    <Screen>
      <TextField rotulo="Descrição *" value={description} onChangeText={setDescription} placeholder="Resistência blindada" maxLength={PART_FIELD_MAX_LENGTH} />

      <Text style={styles.rotulo}>Localização *</Text>
      <View style={styles.chips}>
        {LOCATIONS.map((loc) => (
          <Pressable key={loc.ref} onPress={() => setLocation(loc.ref)} style={[styles.chip, location === loc.ref && styles.chipSelecionado]}>
            <Text style={[styles.chipTexto, location === loc.ref && styles.chipTextoSelecionado]}>{loc.description}</Text>
          </Pressable>
        ))}
      </View>

      {erro ? <Text style={styles.erro}>{erro}</Text> : null}
      <PrimaryButton titulo="Salvar peça" onPress={salvar} carregando={salvando} style={{ marginTop: spacing.md }} />
    </Screen>
  );
}
