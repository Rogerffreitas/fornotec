...
function toPublicUser(user: { id: number; name: string; username: string; role: Role }): AuthenticatedUser {
...
export class UserInteractor implements UserUseCase {
  ...
  async authenticate(username: string, password: string, role: Role): Promise<AuthResult | undefined> {
